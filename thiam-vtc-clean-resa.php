...
function thiam_vtc_clean_resa_shortcode($atts = []) {
  ...
  $airport_flow = ( $mode === 'airport' && is_page('depose-aeroport') ) ? 'depose' : 'default';
  ...
  <div class="thiam-clean" data-mode="<?php echo esc_attr($mode); ?>" data-airport-flow="<?php echo esc_attr($airport_flow); ?>">
    ...
    <div class="thiam-clean__panel">
      <div class="thiam-clean__form">
        ...
        <?php if ( ! ( $mode === 'airport' && $airport_flow === 'depose' ) ) : ?>
          <label>Départ</label>
          <div class="thiam-clean__field">
            <input type="text" class="thiam-clean-start" placeholder="Adresse de départ" autocomplete="off">
            <div class="thiam-clean-suggestions thiam-clean-start-suggestions" role="listbox"></div>
          </div>
        <?php endif; ?>

        <label><?php echo ( $mode === 'airport' && $airport_flow === 'depose' ) ? 'Adresse de dépôt' : 'Arrivée'; ?></label>
        <div class="thiam-clean__field">
          <input type="text" class="thiam-clean-end" placeholder="<?php echo ( $mode === 'airport' && $airport_flow === 'depose' ) ? 'Adresse de dépôt' : 'Adresse de destination'; ?>" autocomplete="off">
...
add_action('wp_enqueue_scripts', function() {
  ...
  $config = [
    ...
    'airportFlow' => is_page('depose-aeroport') ? 'depose' : 'default',
  ];
...
.thiam-clean__panel{background:#f7f2ee;border:1px solid #e9dfd8;border-radius:16px;padding:20px}
.thiam-clean__form{display:grid;gap:10px}
.thiam-clean__form[hidden]{display:none !important}
.thiam-clean label{font-size:14px}
.thiam-clean__field{position:relative}
.thiam-clean input,.thiam-clean select{height:46px;border:1px solid #ddd;border-radius:10px;padding:0 12px;background:#fff}
...
.thiam-clean-btn{height:48px;border:0;border-radius:12px;background:#3A0713;color:#F9F7F2;font-weight:700;cursor:pointer}
.thiam-clean-btn--link{display:flex;align-items:center;justify-content:center;text-decoration:none}
.thiam-clean-meta{font-size:13px;color:#5b4f4a}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('.thiam-clean').forEach(function (wrap) {
    ...
    var isAirportDepose = mode === 'airport' && airportFlow === 'depose';
    ...
    if (startInput) startInput.addEventListener('input', onStartInput);
    if (endInput) endInput.addEventListener('input', onEndInput);
    ...
    function updateAirportMarker() {
      if (mode !== 'airport' || !airportSelect) return;
    ...
    if (btn) {
      btn.addEventListener('click', function () {
        ...
        if (isAirportDepose) {
          if (!end || !time) {
            meta.textContent = 'Merci de renseigner l\'adresse de dépôt et l\'heure.';
            return;
          }
        ...
        payload.append('start_address', isAirportDepose ? airportText : start);
        payload.append('end_address', end);
